...
query <- function(train, support, confidence, commits, k){
  ...
  while(n <= num_transactions){
    commit_files <-  commits$V1[n];
    files <- strsplit(commit_files, ",")[[1]]
    query <-  commits$V2[n];
    ...
    resultsTest<-c(query, files_exp, rules_list, num_predicted, hits, errors, ap[n])
    finalResults <- data.frame(t(resultsTest))
    write.table(finalResults, "log_reg_query.csv", append=TRUE, eol = "\n", sep=";", col.names = F, row.names = F)
